...
export function SiteFooter() {
  ...
  return (
    <footer className="bg-white text-[#0C1A2E]">
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-8 px-6 py-12 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-md">
            ...
            <p className="mt-2 text-[14px] leading-relaxed text-zinc-500">
              Join Lumen and run your next class from one clear workspace.
            </p>
          </div>
          <Link
            href="/register"
            className="inline-flex h-11 shrink-0 items-center justify-center bg-[#0C1A2E] px-5 text-sm font-semibold text-white transition-opacity hover:opacity-90"
          >
            Get started
          </Link>
        </div>
              ...
              <ul className="mt-5 space-y-3">
                {col.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-[13px] text-zinc-600 transition-colors hover:text-[#0C1A2E]"
                    >
                      {link.label}
                    </Link>
                  </li>
      ...
      <div className="border-t border-zinc-200">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-6 py-6 text-[12px] text-zinc-400 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
            <Link href="/privacy" className="hover:text-[#0C1A2E]">
              Privacy Policy
            </Link>
            <Link href="/terms" className="hover:text-[#0C1A2E]">
              Terms &amp; Conditions
            </Link>
          </div>
        </div>
